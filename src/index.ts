...
interface VaporPlugin extends Plugin {
...
export const refreshPaths = ["resources/views/**", "app/routes.swift"].filter(
	(path) => fs.existsSync(path.replace(/\*\*$/, ""))
);

/**
 * Vapor plugin for Vite.
 *
...
export default function vapor(
	...
	return [
		resolveVaporPlugin(pluginConfig),
		...(resolveFullReloadConfig(pluginConfig) as Plugin[]),
	];
}

/**
 * Resolve the Vapor Plugin configuration.
 */
function resolveVaporPlugin(pluginConfig: Required<PluginConfig>): VaporPlugin {
				...
				if (isAddressInfo(address)) {
					...
					setTimeout(() => {
						server.config.logger.info(
							`\n  ${colors.red(
								`${colors.bold("VAPOR")} ${vaporVersion()}`
...
function ensureCommandShouldRunInEnvironment(
	...
	if (command === "build" || env.VAPOR_BYPASS_ENV_CHECK === "1") {
		return;
	}

	if (typeof env.CI !== "undefined") {
		throw Error(
			"You should not run the Vite HMR server in CI environments. You should build your assets for production instead. To disable this ENV check you may set VAPOR_BYPASS_ENV_CHECK=1"
		);
	}
}

/**
 * The version of Vapor being run.
 */
function vaporVersion(): string {
	try {
		const lockfile = JSON.parse(
			fs.readFileSync("Package.resolved").toString()
		);

		return (
			lockfile.pins?.find((swiftPackage: { location: string }) =>
				swiftPackage.location.startsWith(
...
/**
 * The version of the Vapor Vite plugin being run.
 */
...
function resolvePluginConfig(
	...
	if (typeof config === "undefined") {
		throw new Error("vapor-vite-plugin: missing configuration.");
	}
	...
	if (typeof config.publicDirectory === "string") {
		...
		if (config.publicDirectory === "") {
			throw new Error(
				"vapor-vite-plugin: publicDirectory must be a subdirectory. E.g. 'public'."
			);
		}
	}
	...
	if (typeof config.buildDirectory === "string") {
		...
		if (config.buildDirectory === "") {
			throw new Error(
				"vapor-vite-plugin: buildDirectory must be a subdirectory. E.g. 'build'."
			);
		}
	}
...
function resolveFullReloadConfig({
	...
	return (config as RefreshConfig[]).flatMap((c) => {
		...
		/** @ts-ignore */
		plugin.__vapor_plugin_config = c;
...
function noExternalInertiaHelpers(
	...
	const pluginNoExternal = ["vapor-vite-plugin"];
